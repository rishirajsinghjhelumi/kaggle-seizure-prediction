...
OUTPUT_FILE = 'submissionFFTMean_48_SVM2.csv';
...
for i = 1: 5
    ...
    train = sprintf('dog%d_train_fft_mean_48', i);
    label = sprintf('dog%d_label', i);
    test = sprintf('dog%d_test_fft_mean_48', i);
...
%     train = sprintf('patient%d_train_fft_mean', i);
%     label = sprintf('patient%d_label', i);
%     test = sprintf('patient%d_test_fft_mean', i);
%     
%     predictions = predictLR(train, label, test);
%     
%     for j = 1: length(predictions)
%         clip{end + 1} = filenames{filenum};
%         filenum = filenum + 1;
